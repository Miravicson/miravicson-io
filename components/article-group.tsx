...
function ArticleGroup({ articles }: Props): ReactElement {
  const groupedArticles = groupBy(articles, (article: IArticle) =>
  ...
  const groupedArticlesByYear: [
    string,
    [Omit<IArticle, "body">, ...Omit<IArticle, "body">[]],
  ][] = orderedEntries
    .filter((entry) => entry[1].length > 0)
    .map(([year, articles]) => [
      year,
      articles as [Omit<IArticle, "body">, ...Omit<IArticle, "body">[]],
    ]);
  return (
